...
async function expandRoutes(routes, to, minTransferTime, finalLeg = false) {
  ...
  for (const route of routes) {
    ...
    const startDT = DateTime.fromMillis(startTS, { zone: 'utc' }).toFormat('yyyy-MM-dd HH:mm:ss');
    const endDT = DateTime.fromMillis(endTS, { zone: 'utc' }).toFormat('yyyy-MM-dd HH:mm:ss');
